...
class Ir_Renstra extends MX_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->module('template');
		$this->load->model('ir_renstra/Ir_Renstra_model', 'main_model', TRUE);
	...
	public function index()
	{
		if($this->input->get('key')){
			...
			$this->load->view('ir_renstra/frontend/index', $data);
			// echo json_encode($data);
		}else{
			if(@$this->session->has_access[0]->nama_app != "Admin"){	
				$this->load->view('site/404');
			}else{
				$this->template->render('ir_renstra/backend/index');
