...
class Menu_level_model extends CI_Model {
    ...
    public function get_by_level($level) {//note: will be unused
		$this->db->select('
    ...
    public function get_by_app($app_id) {//changed with this!
        $this->db->select('
            m.id_menu,
            m.id_parent,
            m.nama,
    ...
    public function delete_by_level($id_level){//note: will be unused
        $this->db->where('id_level', $id_level);
        $this->db->delete('t_menu_level');
    }

    public function delete_by_appid($id_app){//changed with this!
