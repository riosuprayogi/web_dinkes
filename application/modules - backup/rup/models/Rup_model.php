...
class Rup_model extends CI_Model {
	...
	public function insert_dasar_hukum($data){
		...
		$this->db->insert('t_dasar_hukum',$data);
		return $this->db->insert_id();
	}

	public function update_dasar_hukum($where, $data){
		$this->db->set('uby', $this->session->id_user);
		$this->db->set('udd', date('Y-m-d H:i:s'));
		$this->db->update('t_dasar_hukum', $data, $where);
		return $this->db->affected_rows();
	}

	private function _get_query($filter = array()){
		$this->db->select('t_dasar_hukum.*');
		$this->db->from('t_dasar_hukum');
		$this->db->where('trash', '0');
		// $this->db->where('m_user.status', 1);
		if(@$filter['username'])
			$this->db->like('t_dasar_hukum.judul', $filter['username']);
		if(@$filter['nama'])
	...
	public function get_by_id($id){
		$this->db->select('t_dasar_hukum.*');
		$this->db->from('t_dasar_hukum');
		$this->db->where('id_dasar_hukum', $id);
		$query = $this->db->get();
		return $query->row();
	}
