...
<script>
	...
	$(document).ready(function() {
		...
		table = $('#table').DataTable({
			...
			autoWidth: false,
			ajax: {
				url: '<?php echo site_url('flayer/ajax_list') ?>',
				type: 'GET',
				data: function(data) {
					data.filter = {
						'username': $('#filter_flayer').val(),
					};
				}
			},
			language: {
	...
	function edit(id) {
		...
		//Ajax Load data from ajax
		$.ajax({
			url: "<?php echo site_url('flayer/ajax_edit/') ?>/" + id,
			type: "GET",
			dataType: "JSON",
			async: false,
			success: function(data) {

				$('[name="id"]').val(data.id_flayer);
	...
	function save() {
		...
		url = "<?php echo site_url('flayer/ajax_insert') ?>";
		Swal.fire({
	...
	function del(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "<?php echo site_url('flayer/ajax_delete') ?>/" + id,
					type: "GET",
					dataType: "JSON",
					async: false,
					success: function(data) {
...
<div class="content-header">
	<div class="container-fluid">
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">Manajemen Flayer</h1>
			</div>
			<div class="col-sm-6">
			...
			<table id="table" class="table table-striped table-sm table-bordered">
				<thead>
					<tr>
						...
						<td><input style="width:100%;" class="form-control input-sm" type="text" id="filter_flayer" name="filter_flayer" maxlength="100"></td>
						<td></td>
							...
							<div class="row">
								<div class="col-md-12">
									...
									<div class="form-group">
										<label for=""> Flayer</label>
										<input type="text" name="judul" class="form-control" placeholder="Judul Flayer" required>
									</div>
									<!-- <div class="form-group">
										<label>Isi</label>
										<textarea required="required" data-msg="Please write something :)" name="isian" id="isian"
											class="summernote"></textarea>
									</div> -->
									<div class="form-group">
										<label for="">Berkas</label>
										<div class="file d-none"></div>
										<input id="flayer" accept="image/jpeg,image/gif,image/png,application/pdf" name="flayer" type="file" class="file" data-show-caption="true">
