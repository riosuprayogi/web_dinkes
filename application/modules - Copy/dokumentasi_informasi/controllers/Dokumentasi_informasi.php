...
class Dokumentasi_informasi extends MX_Controller
{
	...
	public function index()
	{
		...
		$data["video_dinkes"] = $this->db->query("SELECT * FROM t_video WHERE status = 'show' AND trash='0' ORDER BY tgl_jam DESC LIMIT 8 ")->result();
...
// ================= akhir mitra
		...
		$data['galeri4'] = $this->db->query("SELECT t_foto_galery.*, t_detail_foto_galery.*
			...
			JOIN t_detail_foto_galery ON t_foto_galery.id_galery = t_detail_foto_galery.id_foto_galery
			                                        -- JOIN web_admin ON web_admin.id_admin = web_artikel.id_admin
			                                        WHERE t_foto_galery.status = 'show' AND trash='0'  ORDER BY tgl_jam DESC LIMIT 8")->result_array();
